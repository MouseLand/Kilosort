...
def find_merges(ctx, flag):
    ...
    if not flag:
        ...
        ir.R_CCG = cp.inf * cp.ones(Xsim.shape)
        ir.Q_CCG = cp.inf * cp.ones(Xsim.shape)
        ir.K_CCG = cp.zeros((*Xsim.shape, 2 * nbins + 1))
...
def splitAllClusters(ctx, flag):
    ...
    while ik < Nfilt:
        ...
        c1 = cp.matmul(wPCA, cp.reshape(cp.mean(clp0[ilow, :], 0), 3, -1))
        # the reconstructed mean waveforms for putative cluster 2
        c2 = cp.matmul(wPCA, cp.reshape(cp.mean(clp0[~ilow, :], 0), 3, -1))
        ...
        if (nremove > 0.05) and (min(plow, phigh) > ccsplit) and (
            ...
            ir.dWU = cp.concatenate((ir.dWU, cp.zeros((*ir.dWU.shape[:-1], 1))), axis=2)
            ir.dWU[:, iC[:, iW[ik]], Nfilt] = c2
            ir.dWU[:, iC[:, iW[ik]], ik] = c1

            # the temporal components are therefore just the PC waveforms
            ir.W = cp.concatenate((ir.W, wPCA.reshape(wPCA.shape[0], 1, wPCA.shape[1])), axis=1)
            ...
            ir.simScore = cp.concatenate(
                (ir.simScore, ir.simScore[:, ik].reshape(-1, 1)), axis=1)
            # copy similarity scores from the original
            ir.simScore = cp.concatenate(
                (ir.simScore, ir.simScore[ik, :].reshape(1, -1)), axis=0)
            ...
            ir.iNeigh = cp.concatenate(
                (ir.iNeigh, ir.iNeigh[:, ik].reshape(-1, 1)), axis=0)
            # copy neighbor channel list from the original
            ir.iNeighPC = cp.concatenate(
                (ir.iNeighPC, ir.iNeighPC[:, ik].reshape(-1, 1)), axis=0)
    ...
    # the window
    ir.Wphy = cp.concatenate((cp.zeros((1 + params.nt0min, Nfilt + 1, Nrank)), ir.W), axis=0)
